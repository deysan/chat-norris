...
export const GoogleLogin: React.FC = () => {
  ...
  useEffect(() => {
    if (user && !loading) {
      const isUser = chats?.some((chat) =>
        chat.users.includes(user.user.email || ''),
      );
      ...
      if (!isUser) {
        chatUsers.map((chatUser) =>
          addDoc(collection(firestore, 'chats'), {
            users: [user.user.email, chatUser.email],
            profile: {
              id: chatUser.id,
              email: chatUser.email,
              username: `${chatUser.first_name} ${chatUser.last_name}`,
              photo: chatUser.avatar,
            },
          }),
        );
      }
  ...
  useEffect(() => {
    fetch('https://reqres.in/api/users')
      .then((res) => res.json())
      .then((json) => setChatUsers(json.data))
  ...
  return (
    <>
      ...
      <div className="flex items-center my-4 before:flex-1 before:border-t before:border-gray-300 before:mt-0.5 after:flex-1 after:border-t after:border-gray-300 after:mt-0.5">
        <p className="text-gray-500 text-center text-sm mx-4 mb-0 uppercase">
          or
        </p>
      </div>
    </>
